/**
 * Update data/cards.csv major rows from data/card_db.csv Major:
 * - Single English name and Traditional Chinese name per row
 * - url from card_db; name_zh_cn from Traditional via twToCn
 * Run from project root: node scripts/updateCardsMajorFromCardDb.js
 */
...
function main() {
  ...
  for (let i = 1; i < cardDbLines.length; i++) {
    ...
    if (!url) continue;
    // card_db typos for slug matching (6 -> G, etc.)
    eng = eng.replace(/\bLIGHTNIN6\b/i, "LIGHTNING");
    eng = eng.replace(/\bBREAKIN6\b/i, "BREAKING");
    eng = eng.replace(/\bCOURSIN6\b/i, "COURSING");
    eng = eng.replace(/\bRUMBLIN6\b/i, "RUMBLING");
    eng = eng.replace(/\bSAL\s+TO\b/i, "SAIL TO");
    // card_db uses comma in "X, Y" (e.g. "FLOW LIKE WATER, REACH LIKE AIR") -> treat as space for slug
    eng = eng.replace(/,/g, " ");
    ...
    majorRowsFromDb.push(row);
  }

  const byKey = new Map();
  for (const row of majorRowsFromDb) {
  ...
  console.log("Updated", updated, "major rows in", CARDS_PATH, "from card_db (url, name_zh_tw, name_zh_cn).");
}
