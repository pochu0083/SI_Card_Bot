...
module.exports = {
  ...
  async execute(message, args, Discord) {
    ...
    if (message.channel.id != landingChannel) {
      return console.log(
        "Reaction role was sent to the wrong channel " + message.channel.id,
      );
    }

    const LFGEmote = message.guild.emojis.cache.find(
      (emoji) => emoji.name === "Fast",
    );
    const PBPEmote = message.guild.emojis.cache.find(
      (emoji) => emoji.name === "Slow",
    );
      ...
      .addFields(
        {
          ...
          value:
            " React with Fast" +
            " to receive the @LFG role, then post in " +
            "the LFG Channel to find other players.",
        },
        {
          name: "Play by Post",
          value: "React with Slow Icon" + " to join the play by post section.",
        },
