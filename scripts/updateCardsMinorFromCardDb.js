/**
 * Update data/cards.csv minor rows from data/card_db.csv Minor:
 * - Single English name and Traditional Chinese name per row
 * - url from card_db; name_zh_cn from Traditional via twToCn
 * Run from project root: node scripts/updateCardsMinorFromCardDb.js
 */
...
function main() {
  ...
  for (let i = 1; i < cardDbLines.length; i++) {
    ...
    minorRowsFromDb.push(row);
  }

  const byKey = new Map();
  for (const row of minorRowsFromDb) {
  ...
  console.log("Updated", updated, "minor rows in", CARDS_PATH, "from card_db (url, name_zh_tw, name_zh_cn).");
}
