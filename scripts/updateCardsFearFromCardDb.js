/**
 * Update data/cards.csv fear rows from data/card_db.csv Fear:
 * - Single English name and Traditional Chinese name per row
 * - url from card_db; name_zh_cn from Traditional via twToCn
 * Run from project root: node scripts/updateCardsFearFromCardDb.js
 */
...
function main() {
  ...
  for (let i = 1; i < cardDbLines.length; i++) {
    ...
    fearRowsFromDb.push(row);
  }

  const byKey = new Map();
  for (const row of fearRowsFromDb) {
    if (!byKey.has(row.key)) byKey.set(row.key, row);
    // cards.csv typo: overseas_trade_seem_safer vs card_db "OVerseas TRADE SEEMS SAFER" (db key has "seemssafer")
    if (row.key === "overseastradeseemssafer") {
      byKey.set("overseastradeseemsafer", row);
    }
  }
  ...
  console.log("Updated", updated, "fear rows in", CARDS_PATH, "from card_db (url, name_zh_tw, name_zh_cn).");
}
