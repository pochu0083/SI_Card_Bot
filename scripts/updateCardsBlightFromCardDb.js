/**
 * Update data/cards.csv blight rows from data/card_db.csv Blight:
 * - Single English name and Traditional Chinese name per row
 * - url from card_db; name_zh_cn from Traditional via twToCn
 * Run from project root: node scripts/updateCardsBlightFromCardDb.js
 */
...
function main() {
  ...
  for (let i = 1; i < cardDbLines.length; i++) {
    ...
    blightRowsFromDb.push(row);
  }

  const byKey = new Map();
  for (const row of blightRowsFromDb) {
    if (!byKey.has(row.key)) byKey.set(row.key, row);
    // cards.csv typo: the_border_of_live_and_death -> match THE BORDER OF LIFE AND DEATH
    if (row.key === "theborderoflifeanddeath") {
      byKey.set("theborderofliveanddeath", row);
    }
  }
  ...
  for (let i = 1; i < cardsLines.length; i++) {
    ...
    let cardKey = normalizeToComparable(name);
    let dbRow = byKey.get(cardKey);
    if (!dbRow && cardKey === "theborderofliveanddeath") {
      dbRow = byKey.get("theborderoflifeanddeath");
    }
    if (!dbRow) {
      outLines.push(fields.map(escapeCsvField).join(","));
      continue;
  ...
  console.log("Updated", updated, "blight rows in", CARDS_PATH, "from card_db (url, name_zh_tw, name_zh_cn).");
}
